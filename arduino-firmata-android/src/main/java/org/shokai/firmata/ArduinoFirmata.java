...
public class ArduinoFirmata {
    ...
    public List<UsbSerialPort> getDeviceList() {
    ...
    public void connect(final UsbSerialPort port) throws IOException, InterruptedException {
        try {
            UsbDeviceConnection connection = usbManager.openDevice(port.getDriver().getDevice());
            if (connection == null) {
                throw new IOException("Opening device failed");
            }
            this.port = port;
